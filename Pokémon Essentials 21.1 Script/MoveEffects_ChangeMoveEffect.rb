...
#===============================================================================
class BattleMoveRedirectAllMovesToUser  BattleMove
  def pbEffectGeneral(user)
    user.effects[PBEffectsFollowMe] = 1
    user.allAllies.each do b
      next if b.effects[PBEffectsFollowMe]  user.effects[PBEffectsFollowMe]
      user.effects[PBEffectsFollowMe] = b.effects[PBEffectsFollowMe] + 1
    end
    user.effects[PBEffectsRagePowder] = true if powderMove
...
# (Spotlight)
#===============================================================================
class BattleMoveRedirectAllMovesToTarget  BattleMove
  def canMagicCoat; return true; end

  def pbEffectAgainstTarget(user, target)
    target.effects[PBEffectsSpotlight] = 1
    target.allAllies.each do b
      next if b.effects[PBEffectsSpotlight]  target.effects[PBEffectsSpotlight]
      target.effects[PBEffectsSpotlight] = b.effects[PBEffectsSpotlight] + 1
    end
...
#===============================================================================
class BattleMoveCannotBeRedirected  BattleMove
...
#===============================================================================
class BattleMoveRandomlyDamageOrHealTarget  BattleMove
...
#===============================================================================
# Damages target if target is a foe, or heals target by 12 of its max HP if
# target is an ally. (Pollen Puff)
#===============================================================================
class BattleMoveHealAllyOrDamageFoe  BattleMove
  def pbTarget(user)
    return GameDataTarget.get(NearFoe) if user.effects[PBEffectsHealBlock]  0
    return super
  end
  ...
  def pbFailsAgainstTarget(user, target, show_message)
    ...
    if target.effects[PBEffectsSubstitute]  0 && !ignoresSubstitute(user)
      @battle.pbDisplay(_INTL(But it failed!)) if show_message
      return true
    end
    if !target.canHeal
...
#===============================================================================
# User is Ghost User loses 12 of max HP, and curses the target.
# Cursed Pokémon lose 14 of their max HP at the end of each round.
# User is not Ghost Decreases the user's Speed by 1 stage, and increases the
# user's Attack and Defense by 1 stage each. (Curse)
#===============================================================================
class BattleMoveCurseTargetOrLowerUserSpd1RaiseUserAtkDef1  BattleMove
  ...
  def pbTarget(user)
    if user.pbHasType(GHOST)
      ghost_target = (SettingsMECHANICS_GENERATION = 8)  RandomNearFoe  NearFoe
      return GameDataTarget.get(ghost_target)
    end
    return super
  end
  ...
  def pbFailsAgainstTarget(user, target, show_message)
    if user.pbHasType(GHOST) && target.effects[PBEffectsCurse]
      @battle.pbDisplay(_INTL(But it failed!)) if show_message
      return true
    end
    return false
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    target.effects[PBEffectsCurse] = true
...
#===============================================================================
class BattleMoveEffectDependsOnEnvironment  BattleMove
  ...
  def pbShowAnimation(id, user, targets, hitNum = 0, showAnimation = true)
    ...
    case @secretPower
    when 1  then id = THUNDERSHOCK if GameDataMove.exists(THUNDERSHOCK)
    when 2  then id = VINEWHIP if GameDataMove.exists(VINEWHIP)
    when 3  then id = FAIRYWIND if GameDataMove.exists(FAIRYWIND)
    when 4  then id = CONFUSIO if GameDataMove.exists(CONFUSION)
    when 5  then id = WATERPULSE if GameDataMove.exists(WATERPULSE)
    when 6  then id = MUDSHOT if GameDataMove.exists(MUDSHOT)
    when 7  then id = ROCKTHROW if GameDataMove.exists(ROCKTHROW)
    when 8  then id = MUDSLAP if GameDataMove.exists(MUDSLAP)
    when 9  then id = ICESHARD if GameDataMove.exists(ICESHARD)
    when 10 then id = INCINERATE if GameDataMove.exists(INCINERATE)
    when 11 then id = SHADOWSNEAK if GameDataMove.exists(SHADOWSNEAK)
    when 12 then id = GUST if GameDataMove.exists(GUST)
    when 13 then id = SWIFT if GameDataMove.exists(SWIFT)
    when 14 then id = PSYWAVE if GameDataMove.exists(PSYWAVE)
    end
    super
  end
end
...
#===============================================================================
class BattleMoveHitsAllFoesAndPowersUpInPsychicTerrain  BattleMove
  def pbTarget(user)
    if @battle.field.terrain == Psychic && user.affectedByTerrain
      return GameDataTarget.get(AllNearFoes)
    end
    return super
  end
...
#===============================================================================
# Powders the foe. This round, if it uses a Fire move, it loses 14 of its max
# HP instead. (Powder)
#===============================================================================
class BattleMoveTargetNextFireMoveDamagesTarget  BattleMove
  def ignoresSubstitute(user); return true; end
  def canMagicCoat;            return true; end

  def pbFailsAgainstTarget(user, target, show_message)
    if target.effects[PBEffectsPowder]
      @battle.pbDisplay(_INTL(But it failed!)) if show_message
      return true
    end
    return false
  ...
  def pbEffectAgainstTarget(user, target)
    target.effects[PBEffectsPowder] = true
...
#===============================================================================
class BattleMoveDoublePowerAfterFusionFlare  BattleMove
  def pbChangeUsageCounters(user, specialUsage)
    @doublePower = @battle.field.effects[PBEffectsFusionFlare]
    super
  end
  ...
  def pbEffectGeneral(user)
    @battle.field.effects[PBEffectsFusionBolt] = true
  end
...
#===============================================================================
class BattleMoveDoublePowerAfterFusionBolt  BattleMove
  def pbChangeUsageCounters(user, specialUsage)
    @doublePower = @battle.field.effects[PBEffectsFusionBolt]
    super
  end
  ...
  def pbEffectGeneral(user)
    @battle.field.effects[PBEffectsFusionFlare] = true
  end
...
#===============================================================================
class BattleMovePowerUpAllyMove  BattleMove
  def ignoresSubstitute(user); return true; end

  def pbFailsAgainstTarget(user, target, show_message)
    if target.fainted  target.effects[PBEffectsHelpingHand]
      @battle.pbDisplay(_INTL(But it failed!)) if show_message
      return true
    end
  ...
  def pbEffectAgainstTarget(user, target)
    target.effects[PBEffectsHelpingHand] = true
...
# (Counter)
#===============================================================================
class BattleMoveCounterPhysicalDamage  BattleMoveFixedDamageMove
  def pbAddTarget(targets, user)
    t = user.effects[PBEffectsCounterTarget]
  ...
  def pbFixedDamage(user, target)
    dmg = user.effects[PBEffectsCounter]  2
    dmg = 1 if dmg == 0
    return dmg
  end
end
...
# (Mirror Coat)
#===============================================================================
class BattleMoveCounterSpecialDamage  BattleMoveFixedDamageMove
  def pbAddTarget(targets, user)
    t = user.effects[PBEffectsMirrorCoatTarget]
    return if t  0  !user.opposes(t)
    user.pbAddTarget(targets, user, @battle.battlers[t], self, false)
  end
  ...
  def pbFixedDamage(user, target)
    dmg = user.effects[PBEffectsMirrorCoat]  2
    dmg = 1 if dmg == 0
    return dmg
  end
end
...
#===============================================================================
class BattleMoveCounterDamagePlusHalf  BattleMoveFixedDamageMove
  def pbAddTarget(targets, user)
...
#===============================================================================
class BattleMoveUserAddStockpileRaiseDefSpDef1  BattleMove
  def canSnatch; return true; end

  def pbMoveFailed(user, targets)
    if user.effects[PBEffectsStockpile] = 3
...
#===============================================================================
class BattleMovePowerDependsOnUserStockpile  BattleMove
  def pbMoveFailed(user, targets)
    if user.effects[PBEffectsStockpile] == 0
  ...
  def pbBaseDamage(baseDmg, user, target)
    return 100  user.effects[PBEffectsStockpile]
  end

  def pbEffectAfterAllHits(user, target)
    return if user.fainted  user.effects[PBEffectsStockpile] == 0
    return if target.damageState.unaffected
    @battle.pbDisplay(_INTL({1}'s stockpiled effect wore off!, user.pbThis))
    return if @battle.pbAllFainted(target.idxOwnSide)
    showAnim = true
    if user.effects[PBEffectsStockpileDef]  0 &&
       user.pbCanLowerStatStage(DEFENSE, user, self)
      showAnim = false if user.pbLowerStatStage(DEFENSE, user.effects[PBEffectsStockpileDef], user, showAnim)
    end
    if user.effects[PBEffectsStockpileSpDef]  0 &&
       user.pbCanLowerStatStage(SPECIAL_DEFENSE, user, self)
      user.pbLowerStatStage(SPECIAL_DEFENSE, user.effects[PBEffectsStockpileSpDef], user, showAnim)
    end
    user.effects[PBEffectsStockpile]      = 0
    user.effects[PBEffectsStockpileDef]   = 0
    user.effects[PBEffectsStockpileSpDef] = 0
  end
end
...
#===============================================================================
class BattleMoveHealUserDependingOnUserStockpile  BattleMove
  ...
  def pbEffectGeneral(user)
    hpGain = 0
    case [user.effects[PBEffectsStockpile], 1].max
    ...
    if user.effects[PBEffectsStockpileDef]  0 &&
       user.pbCanLowerStatStage(DEFENSE, user, self)
      if user.pbLowerStatStage(DEFENSE, user.effects[PBEffectsStockpileDef], user, showAnim)
        showAnim = false
      end
    end
    if user.effects[PBEffectsStockpileSpDef]  0 &&
...
#===============================================================================
class BattleMoveGrassPledge  BattleMovePledgeMove
  def initialize(battle, move)
    super
...
#===============================================================================
class BattleMoveFirePledge  BattleMovePledgeMove
  def initialize(battle, move)
    super
    # [Function code to combo with, effect, override type, override animation]
...
#===============================================================================
class BattleMoveWaterPledge  BattleMovePledgeMove
  def initialize(battle, move)
    super
    # [Function code to combo with, effect, override type, override animation]
...
#===============================================================================
class BattleMoveUseLastMoveUsed  BattleMove
  ...
  def initialize(battle, move)
    ...
    if SettingsMECHANICS_GENERATION = 6
      @moveBlacklist += [
  ...
  def pbMoveFailed(user, targets)
    if !@copied_move 
       !GameDataMove.exists(@copied_move) 
       @moveBlacklist.include(GameDataMove.get(@copied_move).function_code)
      @battle.pbDisplay(_INTL(But it failed!))
      return true
    end
    return false
...
#===============================================================================
class BattleMoveUseLastMoveUsedByTarget  BattleMove
  def ignoresSubstitute(user); return true; end
  def callsAnotherMove; return true; end

  def pbFailsAgainstTarget(user, target, show_message)
    if !target.lastRegularMoveUsed 
       !GameDataMove.exists(target.lastRegularMoveUsed) 
       !GameDataMove.get(target.lastRegularMoveUsed).has_flag(CanMirrorMove)
...
# (Me First)
#===============================================================================
class BattleMoveUseMoveTargetIsAboutToUse  BattleMove
  attr_reader moveBlacklist
  ...
  def pbEffectAgainstTarget(user, target)
    user.effects[PBEffectsMeFirst] = true
    user.pbUseMoveSimple(@battle.choices[target.index][2].id)
    user.effects[PBEffectsMeFirst] = false
  end
end
...
#       Pokémon.
#===============================================================================
class BattleMoveUseMoveDependingOnEnvironment  BattleMove
  attr_reader npMove
  ...
  def pbOnStartUse(user, targets)
    ...
    case @battle.field.terrain
    when Electric
      @npMove = THUNDERBOLT if GameDataMove.exists(THUNDERBOLT)
    when Grassy
      @npMove = ENERGYBALL if GameDataMove.exists(ENERGYBALL)
    when Misty
      @npMove = MOONBLAST if GameDataMove.exists(MOONBLAST)
    when Psychic
      @npMove = PSYCHIC if GameDataMove.exists(PSYCHIC)
    else
      try_move = nil
      case @battle.environment
      when Grass, TallGrass, Forest, ForestGrass
        try_move = (SettingsMECHANICS_GENERATION = 6)  ENERGYBALL  SEEDBOMB
      when MovingWater, StillWater, Underwater
        try_move = HYDROPUMP
      when Puddle
        try_move = MUDBOMB
      when Cave
        try_move = (SettingsMECHANICS_GENERATION = 6)  POWERGEM  ROCKSLIDE
      when Rock, Sand
        try_move = (SettingsMECHANICS_GENERATION = 6)  EARTHPOWER  EARTHQUAKE
      when Snow
        try_move = BLIZZARD
        try_move = FROSTBREATH if SettingsMECHANICS_GENERATION == 6
        try_move = ICEBEAM if SettingsMECHANICS_GENERATION = 7
      when Ice
        try_move = ICEBEAM
      when Volcano
        try_move = LAVAPLUME
      ...
      @npMove = try_move if GameDataMove.exists(try_move)
    end
  end

  def pbEffectAgainstTarget(user, target)
    @battle.pbDisplay(_INTL({1} turned into {2}!, @name, GameDataMove.get(@npMove).name))
...
#===============================================================================
class BattleMoveUseRandomMove  BattleMove
  attr_reader moveBlacklist
  ...
  def pbMoveFailed(user, targets)
    ...
    1000.times do
      ...
      move_data = GameDataMove.get(move_id)
...
#===============================================================================
class BattleMoveUseRandomMoveFromUserParty  BattleMove
  attr_reader moveBlacklist
...
#      SwitchOutTargetStatusMove,                        # Roar, Whirlwind    # See below
    ...
    if SettingsMECHANICS_GENERATION = 6
      @moveBlacklist += [
        ...
        TwoTurnAttackInvulnerableRemoveProtections,      # Shadow ForcePhantom Force
  ...
  def pbMoveFailed(user, targets)
    ...
    @battle.pbParty(user.index).each_with_index do pkmn, i
      ...
      next if SettingsMECHANICS_GENERATION = 6 && pkmn.egg
...
#===============================================================================
class BattleMoveUseRandomUserMoveIfAsleep  BattleMove
  attr_reader moveBlacklist
...
#===============================================================================
class BattleMoveBounceBackProblemCausingStatusMoves  BattleMove
  def pbEffectGeneral(user)
    user.effects[PBEffectsMagicCoat] = true
...
#===============================================================================
class BattleMoveStealAndUseBeneficialStatusMove  BattleMove
  def pbEffectGeneral(user)
    user.effects[PBEffectsSnatch] = 1
    @battle.allBattlers.each do b
      next if b.effects[PBEffectsSnatch]  user.effects[PBEffectsSnatch]
      user.effects[PBEffectsSnatch] = b.effects[PBEffectsSnatch] + 1
    end
...
# out. (Mimic)
#===============================================================================
class BattleMoveReplaceMoveThisBattleWithTargetLastMoveUsed  BattleMove
  attr_reader moveBlacklist
  ...
  def pbMoveFailed(user, targets)
    if user.effects[PBEffectsTransform]  !user.pbHasMove(@id)
      @battle.pbDisplay(_INTL(But it failed!))
      return true
    end
    return false
  ...
  def pbFailsAgainstTarget(user, target, show_message)
    lastMoveData = GameDataMove.try_get(target.lastRegularMoveUsed)
    if !lastMoveData 
  ...
  def pbEffectAgainstTarget(user, target)
    user.eachMoveWithIndex do m, i
      next if m.id != @id
      newMove = PokemonMove.new(target.lastRegularMoveUsed)
      user.moves[i] = BattleMove.from_pokemon_move(@battle, newMove)
...
#===============================================================================
class BattleMoveReplaceMoveWithTargetLastMoveUsed  BattleMove
  attr_reader moveBlacklist
