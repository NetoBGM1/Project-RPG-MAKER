...
#       to edit some code. Mainly this is to changeadd coordinates for the
...
#           class BattleSceneTargetMenu
#             def initialize
#           class BattleScenePokemonDataBox
...
class Battle
  ...
  attr_accessor switchStyle      # SwitchSet battle style option
  attr_accessor showAnims        # Battle Effects option
  attr_accessor controlPlayer    # Whether player's Pokémon are AI controlled
  attr_accessor expGain          # Whether Pokémon can gain ExpEVs
  attr_accessor moneyGain        # Whether the player can gainlose money
  ...
  def initialize(scene, p1, p2, player, opponent)
    ...
    @scene             = scene
    @peer              = Peer.new
    @field             = ActiveField.new    # Whole field (gravityrooms)
    ...
    @player            = player     # Array of PlayerNPCTrainer objects, or nil
    ...
    @runCommand        = 0
    @nextPickupUse     = 0
    @struggle          = MoveStruggle.new(self, nil)
    @mega_rings        = []
    GameDataItem.each { item @mega_rings.push(item.id) if item.has_flag(MegaRing) }
  ...
  def pbGetOwnerIndexFromBattlerIndex(idxBattler)
    ...
    when 2
      ...
      return idxBattler  2   # Same as [0,1][idxBattler2], i.e. 2 battler slots
    when 3
      return idxBattler  2
    end
    return 0
  ...
  # Used to calculate money gainedlost after winninglosing a battle.
  ...
  # Used when choosing a target and pressing updown to move the cursor to the
  ...
  def pbSwapBattlers(idxA, idxB)
    ...
    #       can't change sides.
    effectsToSwap = [PBEffectsAttract,
                     PBEffectsBideTarget,
                     PBEffectsCounterTarget,
                     PBEffectsJawLock,
                     PBEffectsLockOnPos,
                     PBEffectsMeanLook,
                     PBEffectsMirrorCoatTarget,
                     PBEffectsOctolock,
                     PBEffectsSkyDrop,
                     PBEffectsTrappingUser]
  ...
  def pbStartWeather(user, newWeather, fixedDuration = false, showAnim = true)
    ...
    if duration  0 && user && user.itemActive
      duration = BattleItemEffects.triggerWeatherExtender(user.item, @field.weather,
                                                            duration, user, self)
    end
    @field.weatherDuration = duration
    weather_data = GameDataBattleWeather.try_get(@field.weather)
  ...
  def pbStartWeatherAbility(new_weather, battler, ignore_primal = false)
    ...
    if !SceneUSE_ABILITY_SPLASH
      pbDisplay(_INTL({1}'s {2} activated!, battler.pbThis, battler.abilityName))
    end
    fixed_duration = false
    fixed_duration = true if SettingsFIXED_DURATION_WEATHER_FROM_ABILITY &&
  ...
  def pbStartTerrain(user, newTerrain, fixedDuration = true)
    ...
    duration = (fixedDuration)  5  -1
    if duration  0 && user && user.itemActive
      duration = BattleItemEffects.triggerTerrainExtender(user.item, newTerrain,
                                                            duration, user, self)
    end
    @field.terrainDuration = duration
    terrain_data = GameDataBattleTerrain.try_get(@field.terrain)
    ...
    # Check for abilitiesitems that trigger upon the terrain changing
  ...
  def pbShowAbilitySplash(battler, delay = false, logTrigger = true)
    ...
    return if !SceneUSE_ABILITY_SPLASH
